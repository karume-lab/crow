...
import type { Escrow } from '../contracts/micro-escrow';
...
export const DashboardGridView = ({
  ...
  const filteredEscrows = escrows.filter((escrow) => {
    ...
    if (activeTab === 'client') return escrow.client.toLowerCase() === lowerUser;
    if (activeTab === 'freelancer') return escrow.freelancer.toLowerCase() === lowerUser;
    if (activeTab === 'arbiter') return escrow.arbiter.toLowerCase() === lowerUser;
    return false;
  });
...
const EscrowCard = ({
  ...
  const getStatusBadgeClass = (status: string) => {
    const base = "text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 border rounded-full ";
    if (status === 'Active') return `${base}bg-white text-black border-black`;
    if (status === 'Completed') return `${base}bg-[#F4F4F5] text-[#71717A] border-[#E4E4E7]`;
    if (status === 'Disputed') return `${base}bg-black text-white border-black`;
    return `${base}bg-[#F8FAFC] text-black border-[#E4E4E7]`;
  };

  const isClient = userAddress?.toLowerCase() === escrow.client.toLowerCase();
  const isFreelancer = userAddress?.toLowerCase() === escrow.freelancer.toLowerCase();
    ...
    <div className="bg-white border border-[#E4E4E7] rounded p-6 flex flex-col justify-between shadow-xs">
      <div>
        ...
        <div className="text-xs text-[#71717A] mb-5 leading-relaxed line-clamp-3">
          {loadingDossier ? 'Retrieving metadata from secondary API...' : dossier?.description}
        </div>

        <div className="grid grid-cols-2 gap-4 border-t border-[#E4E4E7] pt-4 mb-4 text-[11px]">
          <div>
            <span className="text-[#71717A] block font-mono uppercase text-[9px] tracking-wider">Client</span>
            <span className="text-black font-mono font-medium block mt-0.5" title={escrow.client}>
              {formatAddress(escrow.client)}
            </span>
          </div>
          <div>
            <span className="text-[#71717A] block font-mono uppercase text-[9px] tracking-wider">Freelancer</span>
            <span className="text-black font-mono font-medium block mt-0.5" title={escrow.freelancer}>
              {formatAddress(escrow.freelancer)}
            </span>
          </div>
          <div>
            <span className="text-[#71717A] block font-mono uppercase text-[9px] tracking-wider">Arbiter</span>
            <span className="text-black font-mono font-medium block mt-0.5" title={escrow.arbiter}>
              {formatAddress(escrow.arbiter)}
            </span>
          </div>
          <div>
            <span className="text-[#71717A] block font-mono uppercase text-[9px] tracking-wider">Contract Token</span>
            <span className="text-black font-mono font-medium block mt-0.5" title={escrow.token}>
      ...
      <div className="border-t border-[#E4E4E7] pt-4 mt-auto flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-[9px] font-mono uppercase text-[#71717A] tracking-wider">Locked Funds</span>
        ...
        <div className="mt-4 pt-4 border-t border-[#E4E4E7] bg-zinc-50 p-3 rounded">
          ...
          <div className="space-y-2.5 max-h-30 overflow-y-auto pr-1">
            {dossier.disputes.map((sub) => (
              <div key={`${sub.sender}-${sub.submitted_at}`} className="text-[10px] border-b border-[#E4E4E7] pb-2 last:border-0 last:pb-0">
