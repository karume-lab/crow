...
export const ResolveDisputeModal = ({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (shareBigInt < 0n || shareBigInt > totalAmount) {
			setError(
				"Freelancer share must be between 0 and the total locked amount.",
			);
			return;
		}
		...
		if (success) {
			...
			setError(
				"Resolution transaction failed. Make sure you are the designated Arbiter.",
			);
		}
	};
			...
			<div className="bg-white border border-[#E4E4E7] max-w-md w-full p-6 shadow-lg rounded">
				<div className="mb-6">
					...
					<p className="text-xs text-[#71717A] mt-1 leading-relaxed">
						As the designated neutral Arbiter, divide the locked funds between
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					<div>
						...
						<div className="bg-zinc-50 border border-[#E4E4E7] p-4 rounded space-y-4">
							<div className="flex justify-between text-xs font-mono">
								<div>
									<span className="text-[#71717A] block text-[9px] uppercase tracking-wider">
										Freelancer gets
									</span>
									<span className="text-black font-semibold text-sm">
										{freelancerShareNum} SEP-41
									</span>
								</div>
					...
					<div>
						<label
							htmlFor="numerical-share-input"
							className="block text-[11px] uppercase tracking-wider font-bold text-black mb-1"
						>
