...
function App() {
  ...
  const handleCreateEscrow = async (args: {
    ...
    if (metadataSaved) {
      triggerNotification('success', `Escrow #${escrowId} created and metadata synchronized.`);
      refreshEscrows();
      return true;
    } else {
      triggerNotification('error', `Escrow #${escrowId} created but metadata sync failed.`);
      return true;
    }
  };
  ...
  const handleReleaseFunds = async (escrowId: number): Promise<boolean> => {
    ...
    if (success) {
      ...
      triggerNotification('error', `Failed to release funds for Escrow #${escrowId}.`);
    }
    return success;
  };
  ...
  const handleTriggerDispute = async (escrowId: number): Promise<boolean> => {
    ...
    if (success) {
      triggerNotification('success', `Dispute registered for Escrow #${escrowId}.`);
    } else {
      triggerNotification('error', `Failed to flag dispute for Escrow #${escrowId}.`);
    }
    return success;
  };

  const handleResolveDispute = async (escrowId: number, freelancerShare: bigint): Promise<boolean> => {
    const success = await resolveDisputeOnChain(escrowId, freelancerShare);
    if (success) {
      triggerNotification('success', `Dispute resolved for Escrow #${escrowId}.`);
    } else {
      triggerNotification('error', `Failed to resolve dispute for Escrow #${escrowId}.`);
    }
    return success;
  };
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        <div className="flex flex-col space-y-2">
          ...
          <p className="text-sm text-[#71717A] max-w-2xl leading-relaxed">
            Deploy decentralized, multi-actor escrows. Client funds are locked securely within the Soroban ledger environment, with optional arbitration for active dispute resolution.
          </p>
        </div>
        ...
        {combinedError && (
          <div className="bg-zinc-50 border border-black p-4 text-xs font-mono text-black">
            <span className="font-semibold block mb-1">Operational Exception:</span>
            {combinedError}
          </div>
        )}
