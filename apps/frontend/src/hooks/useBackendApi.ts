...
export function useBackendApi() {
  ...
  const saveEscrowMetadata = useCallback(async (
    escrowId: number,
    title: string,
    description: string,
    clientAddress: string
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/escrows/${escrowId}/metadata`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || `Failed to save metadata (Status ${response.status})`);
      }
      return true;
    } catch (err: any) {
  ...
  const submitDisputeEvidence = useCallback(async (
    escrowId: number,
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/escrows/${escrowId}/dispute-evidence`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        const errData = await response.json().catch(() => ({}));
        throw new Error(errData.error || `Failed to submit evidence (Status ${response.status})`);
      }
  ...
  const getEscrowDossier = useCallback(async (
    escrowId: number
  ): Promise<EscrowDossier | null> => {
    setLoading(true);
    setError(null);
    try {
      const metaRes = await fetch(`${API_BASE_URL}/escrows/${escrowId}/metadata`);
      let metadata: EscrowMetadata | null = null;
      if (metaRes.ok) {
        metadata = await metaRes.json();
      }

      const dispRes = await fetch(`${API_BASE_URL}/escrows/${escrowId}/disputes`);
